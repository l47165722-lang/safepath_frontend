...
fun SafePathMapboxView(
    ...
    // Loading the two CSV files is asynchronous. Keep the latest toggle value so an
    // older load cannot add facility layers again after the user has hidden them.
    val latestShowSafetyFacilities by rememberUpdatedState(showSafetyFacilities)
            ...
            mapView.mapboxMap.getStyle { style ->
                CoroutineScope(Dispatchers.Main).launch {
                    if (showSafetyFacilities) {
                        ...
                        // Resolve the style again after the suspend calls: styles can be
                        // recreated while data is loading.  More importantly, update an
                        // existing source instead of retaining a partial/stale collection.
                        mapView.mapboxMap.getStyle { currentStyle ->
                            if (!latestShowSafetyFacilities) return@getStyle

                            currentStyle.getSourceAs<GeoJsonSource>("cctv-source")
                                ?.featureCollection(cctvGeoJson)
                                ?: currentStyle.addSource(geoJsonSource("cctv-source") {
                                    featureCollection(cctvGeoJson)
                                })
                            if (currentStyle.getLayer("cctv-layer") == null) {
                                currentStyle.addLayer(circleLayer("cctv-layer", "cctv-source") {
                                    circleColor("#2563EB")
                                    circleRadius(4.0)
                            ...
                            currentStyle.getSourceAs<GeoJsonSource>("light-source")
                                ?.featureCollection(lightGeoJson)
                                ?: currentStyle.addSource(geoJsonSource("light-source") {
                                    featureCollection(lightGeoJson)
                                })
                            if (currentStyle.getLayer("light-layer") == null) {
                                currentStyle.addLayer(circleLayer("light-layer", "light-source") {
                                    circleColor("#F59E0B")
                                    circleRadius(2.5)
                                    circleOpacity(0.8)
                                })
